...
function M.query_directory(dir, cb)
  ...
  vim.ui.select(choices, {
    ...
    elseif choice == "Choose current dir" then
      ...
      M.query_directory(dir .. "/" .. choice, cb)
    end
  end)
end
...
--- Generates an ID for an Obsidian note. If timestamp is set, suffixes the
--- title with a timestamp.
--- @param title string The title of the note.
--- @param timestamp boolean Whether to suffix the title with a timestamp.
--- @return string The generated note ID.
function M.note_id(title, timestamp)
  title = M.normalize_note_title(title)
  if not timestamp then
    return title
  end
  return title .. "-" .. tostring(os.time())
end
